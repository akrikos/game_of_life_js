var GridFactory = {
	buildGrid: function(input) {
		...
		var cellList = [],
			cellGrid = [],
			textGrid = this.buildTextGrid(input);
		if (!textGrid.length) {
	...
	_buildTextGrid: function (input) {
		...
		for (var i = 0; i < textGrid.length; i++) {
			...
			if (textGrid[i].length !== previousGridWidth && previousGridWidth !== -1) {
				throw new GenericException('Invalid input. Input grid must be rectangular. Row ' + (i + 1) + ' was a different length from the previous rows');
			}
